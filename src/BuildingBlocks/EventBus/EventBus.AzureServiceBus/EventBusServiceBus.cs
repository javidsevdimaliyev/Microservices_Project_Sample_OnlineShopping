...
public class EventBusServiceBus : BaseEventBus
{
    ...
    {
        // Ex: OrderCreatedIntegrationEvent
    ...
    private void RegisterSubscriptionClientMessageHandler(ISubscriptionClient subscriptionClient)
    {
        ...
        (
            async (message, token) =>
            {
                ...
                if (await HandleEvent(TrimEventName(eventName), messageData))
                {
    ...
    private SubscriptionClient CreateSubscriptionClientIfNotExist(string eventName)
    {
        ...
        var exists = managementClient.SubscriptionExistsAsync(EventBusConfig.DefaultTopicName, GetQueueName(eventName)).GetAwaiter().GetResult();

        if (!exists)
        {
            managementClient.CreateSubscriptionAsync(EventBusConfig.DefaultTopicName, GetQueueName(eventName)).GetAwaiter().GetResult();
            RemoveDefaultRule(subClient);
        }

        CreateRuleIfNotExists(TrimEventName(eventName), subClient);
    ...
    private void CreateRuleIfNotExists(string eventName, ISubscriptionClient subscriptionClient)
    {
        ...
        try
        {
            var rule = managementClient.GetRuleAsync(EventBusConfig.DefaultTopicName, GetQueueName(eventName), eventName).GetAwaiter().GetResult();
    ...
    {
        return new SubscriptionClient(EventBusConfig.EventBusConnectionString, EventBusConfig.DefaultTopicName, GetQueueName(eventName));
    }
